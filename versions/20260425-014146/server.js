...
<body>
      ...
      <div class="section-block hidden" id="section-case-lab">
        <div class="panel">
          ...
          <div class="row" style="margin-bottom:10px">
            ...
            <input id="case-lab-spins" type="number" min="100" max="50000" step="100" value="5000" style="width:120px" title="Количество симуляций">
          ...
          <div class="case-lab-layout">
            <div class="table-wrap" style="max-height:420px">
              <table id="case-lab-table">
                <thead><tr><th>Скин</th><th>Редкость</th><th>Цена $</th><th>Вес</th><th>Шанс %</th></tr></thead>
                <tbody></tbody>
              </table>
            </div>
            <div>
  ...
  <script>
    ...
    function setSection(section){
      const map = ['overview','security','funnel','users','balance','cases','case-lab','promocodes','payments','admins'];
    ...
    function renderCases(rows){
      ...
      body.innerHTML = rows.map(c => {
        ...
        return '<tr>' +
          ...
          '<td>'+esc(fmtTs(c.updatedAt))+'<div class="hint">'+esc(c.updatedBy || '-')+' · '+(c.hasCustomDropTable ? 'custom drop' : 'default drop')+'</div></td>' +
        '</tr>';
      }).join('');
    }
    ...
    function collectCaseLabDraft(){
      ...
      for (const tr of rows) {
        ...
        const weightInput = tr.querySelector('input[data-drop-weight]');
    ...
    function renderCaseLab(data){
      ...
      body.innerHTML = rows.map((r) => '' +
        '<tr data-skin-key="'+esc(r.key)+'">' +
          ...
          '<td><input data-drop-weight="'+esc(r.key)+'" type="number" min="0" step="0.1" value="'+esc(r.weight)+'" style="width:100px"></td>' +
          '<td>'+esc(r.chance)+'%</td>' +
        '</tr>'
      ).join('');
      const lockRows = () => {
        const disabled = !useCustomInput.checked;
        document.querySelectorAll('input[data-drop-weight]').forEach((el) => {
    ...
    async function loadCaseLab(caseName, withSimulation = false){
      ...
      if (withSimulation) {
        ...
        const sim = await jfetch('/admin/api/case-lab/' + encodeURIComponent(key) + '/simulate', {
          ...
          body: JSON.stringify({ spins: Number(document.getElementById('case-lab-spins')?.value || 5000), draft }),
        });
    ...
    async function applyCasePreset(){
      ...
      document.querySelectorAll('input[data-drop-weight]').forEach((el) => {
        const key = el.getAttribute('data-drop-weight');
        const row = byKey[key] || null;
        el.value = String(row?.weight ?? 0);
        el.disabled = false;
      });
      const meta = document.getElementById('case-lab-meta');
      if (meta) {
    ...
    document.getElementById('btn-case-lab-simulate').onclick = async () => {
      ...
      try {
        const data = await jfetch('/admin/api/case-lab/' + encodeURIComponent(draft.caseName) + '/simulate', {
          ...
          headers:{'content-type':'application/json'},
          body: JSON.stringify({
            spins: Number(document.getElementById('case-lab-spins')?.value || 5000),
            draft,
          }),
        });
        renderCaseLab({
...
app.get('/admin/api/case-lab/:caseName', ensureAdminAuth(), async (req, res) => {
  try {
    ...
    cleanup(db);
    normalizeAllConfig(db);
    const cfg = db.caseConfig?.[caseName] || null;
    if (!cfg) return res.status(404).json({ ok: false, error: 'case config missing' });
...
app.post('/admin/api/case-lab/:caseName', ensureAdminAuth(), async (req, res) => {
  try {
    ...
    db.caseConfig[caseName].price = caseName === 'free' ? 0 : Number(nextPriceRaw);
    db.caseConfig[caseName].enabled = Boolean(nextEnabled);
    db.caseConfig[caseName].dropTable = parsedDropTable.table;
    db.caseConfig[caseName].updatedAt = nowIso();
    db.caseConfig[caseName].updatedBy = req.admin?.login || 'admin';
    const sim = runCaseSimulation(db.caseConfig[caseName], 5000);
    appendAdminAudit(db, {
      action: 'case_lab_update',
      reason: `${caseName} price=${db.caseConfig[caseName].price} enabled=${db.caseConfig[caseName].enabled} custom=${Boolean(parsedDropTable.table)} rtp=${sim.rtpPct}%`,
      adminLogin: req.admin?.login || 'admin',
    });
    if (!await persistDbOr503(res, db)) return;
    const dropRows = getCaseDropTableRows(db.caseConfig[caseName]);
    return res.json({
      ok: true,
      case: {
        caseName,
        price: Number(db.caseConfig[caseName].price || 0),
        enabled: Boolean(db.caseConfig[caseName].enabled !== false),
        useCustomDropTable: dropRows.useCustom,
        updatedAt: db.caseConfig[caseName].updatedAt || null,
        updatedBy: db.caseConfig[caseName].updatedBy || null,
      },
      drops: {
        totalWeight: Number(dropRows.totalWeight || 0),
        rows: dropRows.rows,
...
app.post('/admin/api/case-lab/:caseName/simulate', ensureAdminAuth(), async (req, res) => {
  try {
    ...
    const spins = Number(req.body?.spins || 5000);
    const db = await readDb();
    cleanup(db);
    normalizeAllConfig(db);
    const baseCfg = db.caseConfig?.[caseName] || null;
