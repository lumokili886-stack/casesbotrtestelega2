...
<body>
  ...
  <script>
    ...
    function setSection(section){
      ...
      if (sectionSelect && sectionSelect.value !== section) sectionSelect.value = section;
    }
    ...
    async function exportActivityXlsx(){
      ...
      if (typeof XLSX === 'undefined') {
        alert('XLSX библиотека не загружена');
        return;
      }
    ...
    document.getElementById('btn-activity-export').onclick = () => exportActivityXlsx().catch((e)=>alert(e.message));
    document.querySelectorAll('.tab-btn[data-section]').forEach((btn) => {
    ...
    document.getElementById('btn-promo-save').onclick = async () => {
      ...
      if (!expiresLocal) return alert('Укажите срок действия');
