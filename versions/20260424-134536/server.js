...
function parseAdminSession(token) {
  ...
  if (!payload?.login || payload.login !== ADMIN_LOGIN) return { ok: false, error: 'bad login' };
...
function createEmptyDb() {
  return { users: {}, steamLinkRequests: {}, orders: {}, adminAudit: [] };
}
...
<body>
  ...
  <script>
    ...
    function renderKpi(data){
      ...
      document.getElementById('meta-line').textContent = 'Storage: ' + data.storage + ' | path: ' + data.adminPath;
    }
    ...
    async function loadDashboard(){
      ...
      const [overview, users, activity] = await Promise.all([
        ...
        jfetch('/admin/api/activity?limit=200'),
      ]);
      renderKpi(overview);
...
app.get('/admin/api/overview', ensureAdminAuth, async (_req, res) => {
  try {
    ...
    return res.json({
      ...
      kpi: { totalUsers, totalBalance, totalCasesOpened, paidOrders },
    });
  } catch (e) {
...
app.get('/admin/api/activity', ensureAdminAuth, async (req, res) => {
  try {
    ...
    const activity = buildAdminActivity(db, limit);
    const adminAudit = (db.adminAudit || []).slice(-limit).reverse();
    return res.json({ ok: true, activity, adminAudit });
  } catch (e) {
