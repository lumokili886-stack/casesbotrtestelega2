...
const STEAM_REALM = process.env.STEAM_REALM || `${PUBLIC_BASE_URL}/`;
const DATA_PATH = path.resolve(__dirname, process.env.DATA_PATH || './data/db.json');
...
const skinPool = [
  { emoji: '🔫', name: 'USP-S | Cortex', priceUsd: 6, rarity: 'common', r: 'c' },
  { emoji: '🎯', name: 'P2000 | Pulse', priceUsd: 5, rarity: 'common', r: 'c' },
  { emoji: '💣', name: 'P250 | Asiimov', priceUsd: 18, rarity: 'uncommon', r: 'u' },
  { emoji: '⚡', name: 'AWP | Atheris', priceUsd: 22, rarity: 'uncommon', r: 'u' },
  { emoji: '🌊', name: 'M4A4 | Spider Lily', priceUsd: 45, rarity: 'rare', r: 'r' },
  { emoji: '🔧', name: 'M4A1-S | Hyper Beast', priceUsd: 85, rarity: 'rare', r: 'r' },
  { emoji: '🔥', name: 'AK-47 | Fire Serpent', priceUsd: 420, rarity: 'legendary', r: 'l' },
  { emoji: '🔪', name: 'Karambit | Tiger Tooth', priceUsd: 890, rarity: 'legendary', r: 'l' },
];
...
function buildRp(returnUrl) {
  return new RelyingParty(returnUrl, STEAM_REALM, true, false, []);
}
...
app.post('/api/steam/link/start', (req, res) => {
  ...
  auth.db.steamLinkRequests[nonce] = {
  ...
  res.json({ ok: true, steamUrl: `${PUBLIC_BASE_URL}/auth/steam/start?nonce=${encodeURIComponent(nonce)}` });
});
...
app.get('/auth/steam/start', (req, res) => {
  ...
  const db = readDb();
  cleanup(db);
  const reqData = db.steamLinkRequests[nonce];
  if (!reqData) return res.status(400).send('Link request not found or expired');
  writeDb(db);

  const returnUrl = `${PUBLIC_BASE_URL}/auth/steam/return?nonce=${encodeURIComponent(nonce)}`;
  const rp = buildRp(returnUrl);
...
app.get('/auth/steam/return', (req, res) => {
  ...
  rp.verifyAssertion(req, (err, result) => {
    ...
    const userKey = String(reqData.tgUserId);
    if (!db.users[userKey]) db.users[userKey] = makeDefaultUser({ id: reqData.tgUserId });
...
app.listen(PORT, () => {
  ...
  console.log(`[vault] public base: ${PUBLIC_BASE_URL}`);
});
